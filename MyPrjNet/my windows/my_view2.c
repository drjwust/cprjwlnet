...
static void button_view_state(struct rtgui_object *object,
		struct rtgui_event *event)
{
	rtgui_notebook_set_current(the_notebook, RTGUI_WIDGET(Container[3]));
}
		...
		struct rtgui_event *event)
{
	...
	if (listctrl_errornode->current_item < listctrl_errornode->page_items)
		num = 0;
	else
		num = listctrl_errornode->current_item - listctrl_errornode->page_items;
	rtgui_listctrl_set_current_item(listctrl_errornode, num);
}
		...
		struct rtgui_event *event)
{
	uint16_t num;
	num = listctrl_errornode->current_item + listctrl_errornode->page_items;
	if (num >= listctrl_errornode->items_count)
		num = listctrl_errornode->items_count - 1;
	rtgui_listctrl_set_current_item(listctrl_errornode, num);
}
...
/* 创建用于演示label控件的视图 */
rtgui_container_t *my_view2(void)
{
	rtgui_rect_t rect;
	...
	rect.y1 = 75;
	rect.y2 = 250;
	listctrl_errornode = rtgui_listctrl_create(&NodeList->error_node,
			NodeList->error_num , &rect, _rtgui_listctrl_item_draw);
	rtgui_listctrl_set_onitem(listctrl_errornode, on_items);
	/* container是一个container控件，调用add_child方法添加这个listctrl控件 */
	rtgui_container_add_child(container, RTGUI_WIDGET(listctrl_errornode));

...
static rt_bool_t _state_page_refresh_event_handler(struct rtgui_object *object,
		rtgui_event_t *event)
{
	if (event->type == RTGUI_EVENT_PAINT)
	{
		...
		items = listctrl_allnode->items;
		item = &items[listctrl_allnode->current_item];
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_NUM_ID));
		...
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_STATE_ID));
		...
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_TEMP_ID));
		...
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_SIGNAL_ID));
		...
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_R_ALARM_ID));
		...
		{
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_A_ALARM_ID));
		...
		}
		{
			...
			ps[len-2] = '\0';
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_ADDRESS_ID));
		...
		}
		{
			...
			ps[len-2] = '\0';
			/* don't cache the label because it is dynamically created */
			label = RTGUI_LABEL(rtgui_get_self_object(OBJ_LABEL_POPERITY_ID));
			rtgui_label_set_text(label, ps);
			rtgui_widget_update(RTGUI_WIDGET(label));
		}
	}
...
rtgui_container_t *my_view5(void)
{
	rtgui_rect_t rect;
	...
	rtgui_object_set_event_handler(RTGUI_OBJECT(container),
			_state_page_refresh_event_handler);

	/*将这个页面的容器放入到notebook中，以便于翻页*/
	rtgui_notebook_add(the_notebook, "state node", RTGUI_WIDGET(container));
	{
		/*获取页面的绘图区域*/
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_NUM_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_TEMP_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_STATE_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_SIGNAL_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_R_ALARM_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_A_ALARM_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_ADDRESS_ID);
	}
...

	{
		...
		/* container是一个container控件，调用add_child方法添加这个label控件 */
		rtgui_container_add_child(container, RTGUI_WIDGET(label));
		rtgui_object_set_id(RTGUI_OBJECT(label), OBJ_LABEL_POPERITY_ID);
	}
